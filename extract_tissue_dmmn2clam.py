...
def make_sample_grid(slide,patch_size=224,res=20.,min_cc_size=10,max_ratio_size=10,erode=False,prune=False,overlap=1):
    ...
    #img, th = threshold(slide, patch_size, maxres, maxres)
    ...
    #ds_dim = (slide.level_dimensions[slide.get_best_level_for_downsample(maxres/res)][0]/patch_size,slide.level_dimensions[slide.get_best_level_for_downsample(maxres/res)][1]/patch_size)
    #img_ds = cv2.resize(img.astype(int),  (np.ceil(ds_dim[0]).astype(int) , np.ceil(ds_dim[1]).astype(int) ), interpolation=cv2.INTER_NEAREST)
    ##img_ds = skimage.morphology.binary_dilation(img_ds)
    #w = np.where(img_ds > 0)
    #grid = list(zip((w[1] * patch_size * maxres / res).astype(int),
    #                (w[0] * patch_size * maxres / res).astype(int)))
